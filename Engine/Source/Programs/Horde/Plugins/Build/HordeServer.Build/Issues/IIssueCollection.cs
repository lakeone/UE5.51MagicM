...
namespace HordeServer.Issues
{
	...
	public interface IIssueCollection
	{
		...
		/// <param name="options">Options for updating an issue</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>True if the issue was updated</returns>
		Task<IIssue?> TryUpdateIssueAsync(IIssue issue, UserId? initiatedByUserId, UpdateIssueOptions options, CancellationToken cancellationToken = default);
	...
	/// </summary>
	/// <param name="Severity">New severity for the issue</param>
	/// <param name="Summary">New summary for the issue</param>
	/// <param name="UserSummary">New user summary for the issue</param>
	/// <param name="Description">New description for the issue</param>
	/// <param name="Promoted">New promoted state of the issue</param>
	...
	/// <param name="DeclinedById">Name of a user that has declined the issue</param>
	/// <param name="FixCommitId">Fix changelist for the issue. Pass a default value to clear the fix changelist.</param>
	/// <param name="FixSystemic">Whether the issue should be marked fixed as a systemic issue</param>
	/// <param name="ResolvedById">User that resolved the issue (may be ObjectId.Empty to clear)</param>
	/// <param name="ExcludeSpanIds">List of span ids to exclude from this issue</param>
	/// <param name="LastSeenAt"></param>
	/// <param name="ExternalIssueKey">Issue key for external issue tracking</param>
	/// <param name="QuarantinedById">The user that quarantined the issue</param>
	/// <param name="ForceClosedById">The user that force closed the issue</param>
	/// <param name="WorkflowThreadUrl">The workflow thread url associated with the issue</param>
	public record class UpdateIssueOptions
	(
		...
		bool? Promoted = null, 
		UserId? OwnerId = null, 
		UserId? NominatedById = null, 
		bool? Acknowledged = null, 
		UserId? DeclinedById = null, 
		CommitId? FixCommitId = null, 
		bool? FixSystemic = null, 
		UserId? ResolvedById = null, 
		List<ObjectId>? ExcludeSpanIds = null, 
		DateTime? LastSeenAt = null, 
		string? ExternalIssueKey = null, 
		UserId? QuarantinedById = null, 
		UserId? ForceClosedById = null, 
		Uri? WorkflowThreadUrl = null
	);
